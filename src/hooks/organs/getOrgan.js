...
async function getOrgansByCategoryId(organid) {
  try {
    const response = await apiRequest(
      `/v1/organs/get-by-categoryid/${organid}`
    );
...
async function getOrgansDone() {
  try {
    const response = await apiRequest(`/v1/organs/get-all-done`);
...
export { getOrganByOrganId, getOrgansByCategoryId, getOrgansDone };
